...
def get_landmark_list():
    ...
    # Prefer the filtered list (599 curated landmarks) over the full 5064-row file
    for filename in ("Mumbai_Landmarks_filtered.csv", "Mumbai Landmarks.csv"):
        path = os.path.join(base, filename)
        if os.path.exists(path):
            df = pd.read_csv(path)
            df = df.dropna(subset=["latitude", "longitude", "name"])
            tag_col = "tag" if "tag" in df.columns else "category" if "category" in df.columns else None
            def make_label(r):
                if tag_col:
                    tag = str(r.get(tag_col, "")).replace(":", " › ").replace("_", " ")
                    return f"{r['name']}  —  {tag}"
                return r["name"]
...
def render_map_lightweight(steps, start_coords, end_coords) -> io.BytesIO:
    ...
    plt.close(fig)
    plt.close("all")   # kill any lingering figures
    gc.collect()        # force memory release — prevents crash on Streamlit Cloud
    buf.seek(0)
    return buf
